...
const Control = withRouter(observer(({ history, presetIndex, controlIndex, control, hexDisplay, stompswitch=true } = {}) => {
    ...
    const gotoPreset = () => {
        stores.state.selectPreset(presetIndex);
        stores.state.selectControl(controlIndex);
        history.push('/preset');
    }
    ...
    return (
        <div className={`overview-control ${stompswitch?'stompswitch':''} ${stores.state.currentControl === controlIndex ? 'selected' : ''}`} onClick={gotoPreset} title="click to edit">
...
const PresetTitle = withRouter(observer(({ history, presetIndex, presetName} = {}) => {
    ...
    return (
        <div className="preset-title-row">
            <div className="preset-name" onClick={gotoPreset}>
                {presetIndexToXY(presetIndex)}<span className="bullet">•</span><span className="bold as-link">{presetName}</span> {additionalInfos}
            </div>
        </div>
    );
...
export const PresetOverview = observer(({ index, data, hexDisplay, extControls, midi, title }) => {
    ...
    return (
        <div className={`overview-preset ${stores.state.currentPresetIndex === index ? 'selected' : ''}`}>
